...
from cider_ar.cider_ar import CiderRa
...
class RefEvaluation:
    def __init__ (self, refer, Res, eval_cider_r=False):
        """
        :param refer: refer class of current dataset
        :param Res: [{'ref_id', 'sent'}]
        """
        self.evalRefs = []
        self.eval_cider_r = eval_cider_r
        self.eval = {}
        self.refToEval = {}
        self.refer = refer
        self.Res = Res
    ...
    def evaluate(self):
        ...
        for scorer, method in scorers:
            ...
            if method == "R_CIDEr":
                score, scores = scorer.compute_score(self.refToGts, self.refToRes, refToGtRanks1)
            elif method == "Ra_CIDEr":
                score, scores = scorer.compute_score(self.refToGts, self.refToRes, refToGtRanks2)
            else:
